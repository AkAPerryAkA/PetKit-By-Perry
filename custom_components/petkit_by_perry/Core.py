...
async def getAPIToken(Username, Password, Country, TimeZone):
    try:
        ...
        Result = await sendRequest(None, TimeZone, list(dict(API_SERVERS).get(dict(API_COUNTRY).keys()).index(list(dict(API_COUNTRY).values()).index(Country))) + API_LOGIN_PATH, Param)
        Account = {
